...
Use `import.meta.ROLLUP_FILE_URL_<referenceId>`:
...
return `export default import.meta.ROLLUP_FILE_URL_${referenceId} + '#frag'`
```

### In CSS or HTML

`import.meta.ROLLUP_FILE_URL_…` only works in JavaScript expression position. In CSS or HTML, use the `__VITE_ASSET__<referenceId>__` token instead, appending any query or hash right after it:
